...
@section('content')
    <div class="right w-full lg:w-2/3 border border-gray-300 dark:border-gray-700 rounded-2xl lg:p-12 p-6 ">
        <h1 class="text-accentBlackt font-playfair font-bold text-4xl mb-1 dark:text-white">Ubah Password</h1>
        ...
        <form action="{{ route('change.password') }}" method="POST">
            ...
            @foreach ($errors->all() as $error)
                <div class="block">
                    <div class="bg-red-400 rounded-md py-2 px-3 mb-2 inline-block">
                        <p class="text-danger text-white font-poppins font-light text-sm md:text-base ">{{ $error }}
                        </p>
                    </div>
                </div>
            @endforeach
            <div class="flex flex-col md:flex-row mb-1 md:items-center  justify-between md:w-4/5 w-full">
                <label for="password"
                    class="text-left font-poppins font-medium text-base text-accentDarkGray w-full md:w-2/5  dark:text-white">Password
                    Saat ini</label>
                <input id="password" type="password" name="current_password" autocomplete="current-password"
                    class=" px-4 py-2 rounded-md font-poppins font-light w-full md:w-3/5 dark:bg-gray-600 dark:text-white">
            </div>
            <div class="flex flex-col md:flex-row mb-5 md:items-center w-4/5">
                <div class="w-2/5"></div>
                <p class="font-poppins font-light text-sm text-accentDarkGray dark:text-gray-300">Lupa Password?
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}">
                            <span class="text-accentCyan no-underline hover:underline ">Klik Disini
                            </span>
                        </a>
                    @endif
                </p>
            ...
            <div class="flex flex-col md:flex-row mb-5 md:items-center  justify-between md:w-4/5 w-full">
                <label for="confirm_new_password"
                    class="text-left font-poppins font-medium text-base text-accentDarkGray w-full md:w-2/5 dark:text-white">Konfirmasi
                    Password</label>
